...
class TotalSegmentatorIteratorWidget(IteratorWidgetBase):
    ...
    def onInputDirectoryChanged(self, inputDir):
        if inputDir:
            ...
            self.caseFolders = [f for f in os.listdir(inputDir) if os.path.isdir(os.path.join(inputDir, f))]
            self.validate()
    ...
    def startBatch(self, reader=None):
        ...
        if not self.is_valid():
            return None
        self._iterator = TotalSegmentatorIteratorLogic(self.inputDirectory, self.caseFolders, reader, self.segmentNamesEdit.text)
...
class TotalSegmentatorIteratorLogic(IteratorLogicBase):
    def __init__(self, inputDirectory, caseFolders, reader=None, segmentNamesPattern=None):
        super(TotalSegmentatorIteratorLogic, self).__init__()
        self.imageFilename = "ct.nii.gz"
    ...
    def loadCase(self, case_idx):
        ...
        if not os.path.exists(ctPath):
            self.logger.error(f"CT image not found at {ctPath}")
            return None
        ...
        # Load segmentations
        segmentationPath = os.path.join(casePath, "segmentations")
    ...
    def _exportSegments(self):
        ...
        outputDir = os.path.join(self.inputDirectory, self.currentCaseFolder, "segmentations")
        ...
        with slicer.util.RenderBlocker():
            for i in range(segmentation.GetNumberOfSegments()):
                ...
                status = vtk.reference("notstarted")
                segment.GetTag('Segmentation.Status', status)
